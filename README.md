...
сама себя обновить без похода в браузер: скачивает `.exe`, сверяет его
контрольную сумму с той, что GitHub сам посчитал при публикации релиза, и
подменяет запущенный файл. Критическое обновление безопасности показывает
отдельный красный баннер, который не исчезает насовсем, пока не установлено.
...
| **Обновить BridgeBox** | Скачивает, проверяет контрольную сумму и подменяет `.exe`. После успеха предложит перезапуститься - новая версия заработает только после этого. При запуске из исходников недоступно - обновляйтесь через `git pull`. |

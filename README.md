...
> временно антивирус. Без файлов WinDivert обход не запустится, и программа честно
...
3. Первый запуск проведёт через мастер настройки: язык интерфейса, затем подбор
   стратегии обхода, которая реально работает у вашего провайдера.
...
Два способа - в инструкции внутри программы к каждому шагу приложена
короткая анимация.
...
- пингует реальные серверы Jackbox - и Ecast, и Blobcast;
...
языка и подбор стратегии обхода, дальше открывается обычное окно.
...
кнопка проверки соединения, инструкция и бегущая строка благодарностей (клик по
ней открывает список использованных проектов).

### Настройки

Четыре раздела, разобраны [ниже](#все-настройки-по-пунктам).

### Инфо

Логотип, короткое описание программы, текущая версия, статус целостности
файлов, лицензия и кнопки-ссылки (соцсети, донат и т.п.) - у каждой либо
прямой переход по адресу, либо всплывающее окно с текстом. Кнопка-громкоговоритель
в боковой панели ведёт туда же, куда и вкладка «Инфо» - на GitHub Issues или в
форму обратной связи, чтобы сообщить о баге. Содержимое экрана «Инфо» и истории
версий (β-значок выше) - не зашитый в код текст, а данные из
`frontend/src/data/content/`, которые разработчик правит через
`tools/build_content.py`, не трогая исходники руками.
...
Логи параллельно пишутся в `logs/bridgebox.log` с ротацией по 5 МБ.
...
| **Запускать вместе с Windows** | Создаёт задачу в Планировщике заданий с наивысшими правами (это не ключ автозапуска в реестре - без прав администратора обход всё равно не смог бы работать). Рядом - подпункт **«Запускать свёрнутым в трей»**, чтобы окно не появлялось при входе в систему. |
...
| **Папка для временных файлов** | Куда скачивается и распаковывается обновление Zapret (см. ниже). Пусто - системная папка Temp. |
...
| **Тест стратегий** | Перебирает стратегии одну за другой. Можно выбрать, что пинговать - Ecast, Blobcast или оба (тогда это два отдельных прохода по всем стратегиям, один за другим). Потом предлагает применить самую быструю и сохранить результаты в JSON или HTML. Занимает несколько минут, «Оба» - немного дольше. |
...
Это обновление касается только движка обхода. Новую версию самого BridgeBox
нужно скачивать с [Releases](https://github.com/getonjbghelp/bridgebox/releases/latest) -
см. [«Чего BridgeBox пока не умеет»](#чего-bridgebox-пока-не-умеет).
...
- **Не обновляет сам себя.** Сам движок обхода (Zapret) BridgeBox обновляет
  автоматически, а вот новую версию самой программы нужно скачать со страницы
  [Releases](https://github.com/getonjbghelp/bridgebox/releases/latest) и
  заменить папку вручную.
...
на вкладке Releases: чистый `frontend/dist`, `bridgebox.exe` через PyInstaller
(без консольного окна, с запросом прав администратора через встроенный манифест,
с версией, вшитой прямо из `backend/pyproject.toml`), а рядом - актуальный
`zapret/`, пустые `certs/`/`temp/`/`logs/` и чистый `config.yaml`. В конце
скрипт сам проверяет результат: что все нужные файлы на месте, что в exe не
...
умолчанию. Флаг `--skip-frontend-build` пропускает пересборку интерфейса,
если `frontend/dist` уже актуален.
...
BridgeBoxDevVersion/
  ...
  logs/bridgebox.log             журнал работы, ротация по 5 МБ
  ...
  frontend/                      интерфейс на React
    ...
    src/data/strings/ru.json,    тексты интерфейса на двух языках
              en.json
    src/data/content/            история версий и содержимое экрана «Инфо»
...
ссылками в [CREDITS.md](CREDITS.md) и на экране «Инфо» самой программы (клик
по строке «Спасибо …» на главном экране). Ключевые: **zapret** (bol-van),

...
| **Сворачивать в трей при закрытии** | По умолчанию включено. Крестик прячет окно в область уведомлений, а не выключает обход посреди игры. Полный выход - через пункт меню значка в трее. Значок в трее живой: подсказка при наведении и пункт «Остановить мост» всегда показывают актуальное состояние моста. |
...
| **Проверять при запуске** | По умолчанию выключено, как и у Zapret - свежая установка не должна сама стучаться в GitHub. Стоит включить: это тот же канал, по которому приходят критические предупреждения безопасности. |

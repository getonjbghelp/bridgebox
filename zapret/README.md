...
1.10.0 (engine by [bol-van](https://github.com/bol-van/zapret), WinDivert by
...
Pick one in Settings («Стратегия»), grouped **Основная / Альтернативы / Прочие**
by `group_strategies()`. «Тест стратегий» switches zapret to each strategy in
...
**Blobcast** (`blobcast.jackboxgames.com`), or **Оба**, which is two complete
...
`Прочие` group by default - Fake TLS Auto / Simple Fake are slow to probe.
...
Editable from Settings («Сеть и обход» → «Домены для обхода»), which validates
at the boundary and writes atomically: winws does not reject a malformed line,
it silently ignores it, so a bad entry would otherwise surface as "zapret
bypassed nothing" at the next bridge start, far from the edit that caused it.
The list currently covers 12 official Jackbox hosts only - Ecast (4), Blobcast
(3), and shared/CDN hosts (5). Unofficial mirrors (`jackbox.fun` and similar)
are **not** included by default; add them by hand here if needed, the same way
- see the root `README.md`'s troubleshooting section for why they were dropped.
...
Профили подключения → the Blobcast profile → «Порт socket.io»), every strategy
needs the new port added the same way, or that profile loses its bypass
silently. `test_blobcast_port_is_in_the_dpi_filters` pins the port number to
`server/blobcast.py`'s `SOCKETIO_PORT` so a mismatch fails loudly instead.
...
- from Settings → «Обновление Zapret». Pinned to `Flowseal/zapret-discord-youtube`
